...
ax.add_feature(cfeature.LAND, color = '#B7C6CF',alpha = 0.7, zorder = 1)
ax.add_feature(cfeature.BORDERS, color = 'white', zorder = 2)


ax.scatter(lons_survived, lats_survived, s=5 , c ='#3EBCD2', linewidths = 0 ,transform=ccrs.PlateCarree(), zorder = 3, alpha = 1)
ax.scatter(lons_destroyed, lats_destroyed, s=5,c ='#A81829', linewidths = 0 ,transform=ccrs.PlateCarree(), zorder = 4, alpha = 1)
ax.scatter(lons_created, lat_created, s=5, c ='#EBB434', linewidths = 0 ,transform=ccrs.PlateCarree(), zorder = 5, alpha = 1)
ax.axis('off')
...
plt.savefig(r'./outputs/figures/figure-2.pdf',dpi = 600, bbox_inches = 'tight', facecolor = 'white') ## Used for article demonstration